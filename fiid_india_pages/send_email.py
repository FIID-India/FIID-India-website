...
def contact_email(name, message, email_to, email_from, email_subject, email_template):
    html_content = render_to_string(email_template, {
                                    'name': name, 'message': message, 'email': email_from, 'subject': email_subject})
...
# Send subscriber a welcome email
def subscriber_welcome_email(full_name, subject, to_email, email_template):
    html_content = render_to_string(
        email_template, {'full_name': full_name, 'email': to_email})
    text_content = strip_tags(html_content)
    email = EmailMultiAlternatives(
        subject,
        text_content,
...
# Notify Subscriber on new release of report or newsletter
def subscriber_email(full_name, subject, to_email, message, email_template):
    html_content = render_to_string(
        email_template, {'full_name': full_name, 'message': message})
    text_content = strip_tags(html_content)
    email = EmailMultiAlternatives(
        subject,
        text_content,
